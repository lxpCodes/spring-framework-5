...
public class SimpleInstantiationStrategy implements InstantiationStrategy {
	...
	public Object instantiate(RootBeanDefinition bd, @Nullable String beanName, BeanFactory owner) {
		// Don't override the class with CGLIB if no overrides.如果没有重写，不要用CGLIB代理
		// 有需要覆盖或者动态替换的方法需要用cglib进行动态代理,因为可以在创建代理时将动态方法织入类中
		// 没有则直接用反射
	 ...
	 * 那么子类可以覆盖这个方法，实现这个方法是为了抛出UnsupportedOperationException。实例化应该使用无参数的构造函数。
	 */
