...
public abstract class AbstractRefreshableApplicationContext extends AbstractApplicationContext {
	...
	protected final void refreshBeanFactory() throws BeansException {
		...
		try {
			...
			// 根据加载不同配置类型，调用不同实现
			// xml形式调用AbstractXmlApplicationContext 解析xml文件
			// 注解形式调用AnnotationConfigWebApplicationContext
	 ...
	 * 定制此上下文使用的内部bean工厂。
	 * 调尝试用每个{@link #refresh()}。默认实现应用此上下文的{@linkplain #setAllowBeanDefinitionOverriding "allowBeanDefinitionOverriding"}
	 * 和 {@linkplain #setAllowCircularReferences "allowCircularReferences"} 设置
	 * (如果指定)。可以在子类中被重写来定制任何{@link DefaultListableBeanFactory}的设置。
