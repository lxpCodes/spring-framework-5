...
class CglibAopProxy implements AopProxy, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			// Validate the class, writing log messages as necessary. 验证class
	...
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		// Parameters used for optimization choices... 对exposeProxy属性的处理
		boolean exposeProxy = this.advised.isExposeProxy();
		boolean isFrozen = this.advised.isFrozen();
		boolean isStatic = this.advised.getTargetSource().isStatic();

		// Choose an "aop" interceptor (used for AOP calls). 将拦截器封装在DynamicAdvisedInterceptor中
		Callback aopInterceptor = new DynamicAdvisedInterceptor(this.advised);

		// Choose a "straight to target" interceptor. (used for calls that are
		// unadvised but can return this). May be required to expose the proxy.
		// 选择一架“直击目标”的拦截器,(用于未被建议但可以返回此值的调用),可能需要公开代理。
		...
		// 如果目标是静态的，通知链被冻结，那么我们可以通过使用该方法的固定链将AOP调用直接发送到目标来进行一些优化。
		if (isStatic && isFrozen) {
			...
			// 现在将mainCallbacks和fixedCallbacks中的回调都复制到回调数组中。
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			...
			try {
				...
				if (chain.isEmpty() && Modifier.isPublic(method.getModifiers())) {
					...
					// 我们可以跳过创建方法调用:直接调用目标。
					// 请注意，最终的调用程序必须是一个InvokerInterceptor，因此我们知道它只对目标执行反射操作，没有热交换或花哨的代理
